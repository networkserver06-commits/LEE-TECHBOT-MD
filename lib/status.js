...
async function publishStatus(sock, content, recipients) {
    ...
    try {
        ...
        // Some Baileys/WhatsApp sessions reject a native-audience status when
        // no statusJidList is supplied. Retry once with only normal WhatsApp
        // number JIDs; never retry with LIDs or stale group identifiers.
        if (recipients?.length) throw firstError;
        const audience = await personalAudience(sock);
        const validJids = audience.filter((jid) => /@s\.whatsapp\.net$/i.test(jid));
        if (!validJids.length) throw firstError;
        await sock.sendMessage('status@broadcast', payload, { broadcast: true, statusJidList: [...new Set(validJids)] });
    }
}
