...
function getCommandText(message) {
    const value = unwrap(message?.message || message);
    const text = value.conversation || value.extendedTextMessage?.text || '';
    const match = String(text).match(/^\s*[./!]tog?status(?:\s+now)?(?:\s+([\s\S]*))?\s*$/i);
    return match?.[1]?.trim() || null;
}

function getCommandContent(message) {
    return getContent(getQuotedMessage(message)) || getContent(getDirectMediaMessage(message)) || (getCommandText(message) ? { type: 'text', value: getCommandText(message) } : null);
}
