...
const audienceCache = new WeakMap();
const AUDIENCE_CACHE_TTL_MS = Number(process.env.STATUS_AUDIENCE_CACHE_MS || 15000);
const MEDIA_LIMITS = {
    image: Number(process.env.STATUS_MAX_IMAGE_BYTES || 16 * 1024 * 1024),
    video: Number(process.env.STATUS_MAX_VIDEO_BYTES || 64 * 1024 * 1024),
    audio: Number(process.env.STATUS_MAX_AUDIO_BYTES || 32 * 1024 * 1024),
    document: Number(process.env.STATUS_MAX_DOCUMENT_BYTES || 50 * 1024 * 1024)
};
...
function getCommandText(message) {
    const value = unwrap(message?.message || message);
    const mediaCaption = value.imageMessage?.caption || value.videoMessage?.caption || value.audioMessage?.caption || value.documentMessage?.caption || '';
    const text = value.conversation || value.extendedTextMessage?.text || mediaCaption;
    const match = String(text).match(/^\s*[./!]tog?status(?:\s+now)?(?:\s+([\s\S]*))?\s*$/i);
    return match?.[1]?.trim() || null;
}

function getCommandContent(message) {
    const commandText = getCommandText(message);
    const content = getContent(getQuotedMessage(message)) || getContent(getDirectMediaMessage(message));
    if (content?.type !== 'text' && content && commandText) {
        return { ...content, value: { ...content.value, caption: commandText } };
    }
    return content || (commandText ? { type: 'text', value: commandText } : null);
}
...
async function downloadMedia(media, type) {
    ...
    const chunks = [];
    const limit = MEDIA_LIMITS[type] || MEDIA_LIMITS.document;
    let total = 0;
    for await (const chunk of stream) {
        total += chunk.length;
        if (total > limit) {
            stream.return?.();
            throw new Error(`${type} is too large for fast Status upload (maximum ${Math.round(limit / 1024 / 1024)}MB)`);
        }
        chunks.push(chunk);
    }
...
async function personalAudience(sock) {
    const cached = audienceCache.get(sock);
    ...
    audienceCache.set(sock, { recipients: result, expiresAt: Date.now() + AUDIENCE_CACHE_TTL_MS });
    return [...result];
}
...
async function buildStatusPayload(content) {
    ...
    return content.type === 'text'
        ...
        : {
            ...
            ...(content.value.gifPlayback ? { gifPlayback: true } : {}),
            ...(content.value.ptt ? { ptt: true } : {})
        };
}
...
async function publishStatus(sock, content, recipients) {
    ...
    const options = { broadcast: true, statusJidList: validJids.length ? validJids : lidJids };
    try {
        ...
        await sock.sendMessage('status@broadcast', payload, { broadcast: true, statusJidList: lidJids });
    }
}

async function publishToChat(sock, chatId, content, mentions = []) {
    const payload = await buildStatusPayload(content);
    const validMentions = [...new Set(mentions.filter((jid) => /@s\.whatsapp\.net$/i.test(jid)))];
    if (validMentions.length) {
        const mentionText = validMentions.map((jid) => `@${jid.split('@')[0]}`).join(' ');
        if (payload.text) payload.text = `${payload.text}\n\n👥 ${mentionText}`;
        else payload.caption = `${payload.caption ? `${payload.caption}\n\n` : ''}👥 ${mentionText}`;
