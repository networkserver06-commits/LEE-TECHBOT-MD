...
function createPairingWebServer({ getSocket, enabled = true, host = '127.0.0.1', port = 3000, token = '', logger = console } = {}) {
    ...
    if (host !== '127.0.0.1' && host !== 'localhost' && !token) {
        throw new Error('PAIRING_WEB_TOKEN is required when PAIRING_WEB_HOST is public');
    }
    const rate = new Map();
    ...
    server.listen(Number(port), host, () => logger.log?.(`Pairing website available at http://${host}:${port}`));
    return server;
}
