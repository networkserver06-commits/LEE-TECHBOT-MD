...
function connectionSnapshot(socket) {
    ...
    // Baileys may not expose the underlying WebSocket as readyState=1 even
    // after connection.update has reported `open`. The lifecycle flag is set
    // by the bot's authoritative connection handler and must win here.
    const ready = socket?.__connectionOpened === true || socket?.ws?.readyState === 1;
...
function createPairingWebServer({ getSocket, enabled = true, host = '127.0.0.1', port = 3000, token = '', authDir = process.env.AUTH_DIR || './session', logger = console } = {}) {
    ...
    const server = http.createServer(async (req, res) => {
        ...
        if (req.method === 'GET' && route === '/api/auth-state') {
            return json(res, 200, { passwordConfigured: Boolean(readPasswordRecord(passwordFile)) });
        }
        ...
        if (socket.authState?.creds?.registered) {
            return json(res, 409, { paired: true, status: connectionSnapshot(socket), error: 'This WhatsApp account is already linked. Showing connection status instead of requesting another pairing code.' });
        }
        ...
        try {
            const code = await requestPairingCodeWithRetry({
                socket,
                getSocket,
                onTransientError: async (_error, failedSocket) => {
                    try {
                        if (failedSocket?.ws && typeof failedSocket.ws.close === 'function') failedSocket.ws.close();
                    } catch (_) {}
                },
                phoneNumber,
                isActive: () => {
                    const current = getSocket?.();
                    return Boolean(current && !current.authState?.creds?.registered);
                },
                logger
            });
