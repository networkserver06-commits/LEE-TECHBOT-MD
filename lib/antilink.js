...
async function Antilink(msg, sock) {
    ...
    try {
        ...
        // Silent mode normally ends after deletion. A ban action is different:
        // it must continue counting violations, but suppresses intermediate
        // warning messages until the repeat-offender threshold is reached.
        ...
        if (action === 'kick') {
            ...
            if (warningCount < WARN_COUNT) {
                if (!settings.silent) await sock.sendMessage(jid, { text: `@${sender.split('@')[0]} warning ${warningCount}/${WARN_COUNT} for sending an unauthorized link.`, mentions: [sender] });
                return;
            }
            const banned = await addBannedUser(sender);
            if (!banned) {
                await resetWarningCount(jid, sender);
                await sock.sendMessage(jid, { text: `@${sender.split('@')[0]} reached ${WARN_COUNT} blocked-link warnings, but the global ban could not be saved.`, mentions: [sender] });
                return;
            }
            await sock.groupParticipantsUpdate(jid, [sender], 'remove');
            await resetWarningCount(jid, sender);
            await sock.sendMessage(jid, { text: `@${sender.split('@')[0]} was globally banned and removed after ${WARN_COUNT} unauthorized links.`, mentions: [sender] });
        } else if (action === 'warn') {
            const warningCount = await incrementWarningCount(jid, sender);
            if (warningCount >= WARN_COUNT) {
                await sock.groupParticipantsUpdate(jid, [sender], 'remove');
