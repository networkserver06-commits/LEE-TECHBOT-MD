...
function loadUserGroupData() {
    try {
        ...
        writeJsonAtomic(dataPath, defaultData);
            return defaultData;
        }
...
async function setAntilink(groupId, type, action, options = {}) {
    try {
        const data = loadUserGroupData();
        if (!data.antilink) data.antilink = {};
        if (!data.antilink[groupId]) data.antilink[groupId] = {};
        
        const previous = data.antilink[groupId] || {};
        const requestedThreshold = Number(options.threshold);
        const previousThreshold = Number(previous.threshold);
        const defaultThreshold = Math.max(1, Number(process.env.WARN_COUNT || 3));
        const threshold = Number.isInteger(requestedThreshold) && requestedThreshold >= 1 && requestedThreshold <= 100
            ? requestedThreshold
            : (Number.isInteger(previousThreshold) && previousThreshold >= 1 && previousThreshold <= 100 ? previousThreshold : defaultThreshold);
        data.antilink[groupId] = {
            ...previous,
            enabled: type === 'on',
            action: action || previous.action || 'delete',
            mode: options.mode || previous.mode || 'all',
		    silent: options.silent === undefined ? previous.silent === true : options.silent === true,
		    threshold,
            allowDomains: Array.isArray(options.allowDomains) ? options.allowDomains : (Array.isArray(previous.allowDomains) ? previous.allowDomains : []),
            denyDomains: Array.isArray(options.denyDomains) ? options.denyDomains : (Array.isArray(previous.denyDomains) ? previous.denyDomains : [])
        };
        if (options.enforceAdmins === true || previous.enforceAdmins === true) {
            data.antilink[groupId].enforceAdmins = options.enforceAdmins === undefined ? true : options.enforceAdmins === true;
        }
...
async function removeAntilink(groupId, type) {
    try {
        ...
        // Do not carry old violation counts into a future anti-link policy.
        // The settings and enforcement state are persisted together.
        if (data.warnings && data.warnings[groupId]) delete data.warnings[groupId];
        saveUserGroupData(data);
        return true;
    } catch (error) {
...
async function addBannedUser(userId) {
    try {
        ...
        writeJsonAtomic(bannedPath, bannedUsers);
        return true;
    } catch (error) {
...
async function addWelcome(jid, enabled, message) {
    try {
        ...
        data.welcome[jid] = {
            ...
            channelId: '120363404186001130@newsletter'
        };
