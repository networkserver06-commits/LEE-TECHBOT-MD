...
- **Complete chat modes:** `.mode public`, `.mode group`, `.mode dm`, and `.mode private` are persisted. `public` accepts groups and DMs, `group` accepts groups only, `dm` accepts private chats only, and `private` remains the legacy strict group-only mode.
